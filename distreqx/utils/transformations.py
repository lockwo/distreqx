...
def inv(fun: Callable[[PyTree], PyTree]) -> Callable[[PyTree], PyTree]:
    ...
    **Arguments:**
    ...
    - `fun`: The function to invert. It must be composed of invertible primitives.

    **Returns:**

    - A callable that computes the inverse of `fun`.
    """
...
def log_det_scalar(fn: Callable[[PyTree], PyTree]) -> Callable[[PyTree], PyTree]:
    ...
    **Arguments:**
    ...
    - `fn`: The scalar function to derive the log-determinant for.

    **Returns:**

    - A callable that computes the log-determinant of the Jacobian of `fn`.
    """
    _check_numerical_stability(fn)
    jac_fn = jax.vmap(jax.jacfwd(fn))

    def log_det_fn(x: Array) -> Array:
        x = jnp.asarray(x)
...
def _check_numerical_stability(fn: Callable[[PyTree], PyTree]) -> None:
    ...
    for eqn in jaxpr.eqns:
        if eqn.primitive in _potentially_unstable_primitives:
            warnings.warn(
                f"[distreqx]: the '{eqn.primitive}' primitive can exhibit unstable "
                "numerical behavior under certain circumstances. Consider "
                f"using the {_potentially_unstable_primitives[eqn.primitive]} "
                "bijector instead if possible."
            )
...
def _identify_variable_in_eqn(eqn) -> int:
