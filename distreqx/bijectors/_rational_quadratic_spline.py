...
def _normalize_bin_sizes(
    ...
    if num_bins * min_bin_size > total_size:
        raise ValueError(
            ...
            f"({min_bin_size}) cannot be greater than the total bin size ({total_size})."
        )
...
def _normalize_knot_slopes(unnormalized_knot_slopes: Array, min_knot_slope: float) -> Array:
    ...
    if min_knot_slope >= 1.0:
        raise ValueError(f"The minimum knot slope must be less than 1; got {min_knot_slope}.")
...
def _rational_quadratic_spline_fwd(
    ...
    logdet = 2.0 * jnp.log(bin_slope) + jnp.log(
        knot_slopes_bin[1] * sq_z + 2.0 * bin_slope * z1mz + knot_slopes_bin[0] * sq_1mz
...
class RationalQuadraticSpline(
    ...
    def __init__(
        ...
        bin_widths = _normalize_bin_sizes(unnormalized_bin_widths, range_size, min_bin_size)
        bin_heights = _normalize_bin_sizes(unnormalized_bin_heights, range_size, min_bin_size)
        ...
        knot_slopes_core = _normalize_knot_slopes(unnormalized_knot_slopes, min_knot_slope)
        ...
        if boundary_slopes == "unconstrained":
            ...
            self.knot_slopes = jnp.concatenate([ones, knot_slopes_core[..., 1:]], axis=-1)
        elif boundary_slopes == "upper_identity":
            ones = jnp.ones(pad_shape, dtype)
            self.knot_slopes = jnp.concatenate([knot_slopes_core[..., :-1], ones], axis=-1)
        elif boundary_slopes == "identity":
            ones = jnp.ones(pad_shape, dtype)
            self.knot_slopes = jnp.concatenate([ones, knot_slopes_core[..., 1:-1], ones], axis=-1)
        elif boundary_slopes == "circular":
            self.knot_slopes = jnp.concatenate(
                [knot_slopes_core[..., :-1], knot_slopes_core[..., :1]], axis=-1
            )
        else:
            raise ValueError(f"Unknown option for boundary slopes: `{boundary_slopes}`.")

    def forward_and_log_det(self, x: Array) -> tuple[Array, Array]:
        fn = jnp.vectorize(_rational_quadratic_spline_fwd, signature="(),(n),(n),(n)->(),()")
        y, logdet = fn(x, self.x_pos, self.y_pos, self.knot_slopes)
        return y, logdet

    def inverse_and_log_det(self, y: Array) -> tuple[Array, Array]:
        fn = jnp.vectorize(_rational_quadratic_spline_inv, signature="(),(n),(n),(n)->(),()")
